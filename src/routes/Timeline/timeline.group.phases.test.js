...
describe('timeline @ phases', function () {
    ...
    it('should merge car trip phases when stopping on MOP', function () {
        ...
        const tripEvents = [
            createTransportEvent(TRANSPORT_MODE.CAR, getCheckinDate(car_mop), getCheckinLocation(car_mop), getCheckinLocation(restaurant_limassol_cyprus)),
            car_mop,
            createTransportEvent(TRANSPORT_MODE.CAR, getCheckinDate(airport_cyprus), getCheckinLocation(airport_cyprus), getCheckinLocation(car_mop)),
            createCheckinEvent(airport_cyprus),
            createTransportEvent(TRANSPORT_MODE.CAR, getCheckinDate(fitness_wroclaw), getCheckinLocation(fitness_wroclaw), getCheckinLocation(airport_cyprus))
        ]
        const phases = createPhasesWithEvents(tripEvents)
        const expectedPhases = [
            createTransportEvent(TRANSPORT_MODE.CAR, getCheckinDate(airport_cyprus), getCheckinLocation(airport_cyprus), getCheckinLocation(restaurant_limassol_cyprus), true),
            createCheckinEvent(airport_cyprus),
            createTransportEvent(TRANSPORT_MODE.CAR, getCheckinDate(fitness_wroclaw), getCheckinLocation(fitness_wroclaw), getCheckinLocation(airport_cyprus))
        ]
        assert.deepEqual(phases, expectedPhases)
