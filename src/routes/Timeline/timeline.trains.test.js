...
describe('timeline @ trains', function () {
    it('should create train transport event with destination train station', function () {
        ...
        const expectedEvents = [
            ...
            createTransportEvent(TRANSPORT_MODE.TRAIN, getCheckinLocation(fitness_wroclaw), getCheckinLocation(trainstation_krakow)),
    ...
    it('should create train transport event with departure train station', function () {
        ...
        const expectedEvents = [
            createCheckinEvent(fitness_wroclaw),
            createTransportEvent(TRANSPORT_MODE.TRAIN, getCheckinLocation(trainstation_krakow), getCheckinLocation(fitness_wroclaw)),
            createCheckinEvent(trainstation_krakow),
        ]
        assert.deepEqual(events, expectedEvents)
    })
    it('should create train transport event with both train stations', function () {
        ...
        const expectedEvents = [
            ...
            createTransportEvent(TRANSPORT_MODE.TRAIN, getCheckinLocation(trainstation_krakow), getCheckinLocation(trainstation_wroclaw)),
            createCheckinEvent(trainstation_krakow),
        ]
        assert.deepEqual(events, expectedEvents)
    })
